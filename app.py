...
app.config['SQLALCHEMY_DATABASE_URI'] = 'sqlite:///' + os.path.join(BASE_DIR, 'citas_sqlalchemy.db')
...
def cargar_usuario(id_usuario):
    datos = obtener_usuario_por_id(int(id_usuario))
    if datos:
        return Usuario(
...
class GestorInventario:
        ...
        with sqlite3.connect(self.DB_PATH) as conn:
            conn.cursor().executescript("""
                CREATE TABLE IF NOT EXISTS pacientes (
                    ...
                    telefono TEXT, correo TEXT, fecha_nacimiento TEXT);
                ...
                CREATE TABLE IF NOT EXISTS citas (
                    ...
                    fecha TEXT NOT NULL, hora TEXT NOT NULL, motivo TEXT,
    ...
    def _cargar_datos(self):
        with sqlite3.connect(self.DB_PATH) as conn:
            ...
            for row in cursor.execute("SELECT * FROM pacientes"):
                p = Paciente(**dict(row)); self._pacientes[p.id_paciente] = p
            for row in cursor.execute("SELECT * FROM doctores"):
                d = Doctor(**dict(row)); self._doctores[d.id_doctor] = d
            for row in cursor.execute("SELECT * FROM citas"):
                c = Cita(**dict(row)); self._citas[c.id_cita] = c
    def agregar_paciente(self, nombre, cedula, telefono, correo, fecha_nacimiento):
        with sqlite3.connect(self.DB_PATH) as conn:
            cursor = conn.cursor()
            cursor.execute("INSERT INTO pacientes (nombre,cedula,telefono,correo,fecha_nacimiento) VALUES (?,?,?,?,?)",
                           (nombre, cedula, telefono, correo, fecha_nacimiento))
            conn.commit(); nuevo_id = cursor.lastrowid
        p = Paciente(nuevo_id, nombre, cedula, telefono, correo, fecha_nacimiento)
        self._pacientes[nuevo_id] = p; return p
    def obtener_paciente(self, id_paciente): return self._pacientes.get(id_paciente)
    def listar_pacientes(self): return list(self._pacientes.values())
    ...
    def actualizar_paciente(self, id_paciente, nombre, telefono, correo):
        if id_paciente not in self._pacientes: return False
        with sqlite3.connect(self.DB_PATH) as conn:
            conn.execute("UPDATE pacientes SET nombre=?,telefono=?,correo=? WHERE id_paciente=?",
                         (nombre, telefono, correo, id_paciente)); conn.commit()
        p = self._pacientes[id_paciente]
        p.nombre = nombre; p.telefono = telefono; p.correo = correo; return True
    def eliminar_paciente(self, id_paciente):
        if id_paciente not in self._pacientes: return False
        with sqlite3.connect(self.DB_PATH) as conn:
            conn.execute("DELETE FROM citas WHERE id_paciente=?", (id_paciente,))
            conn.execute("DELETE FROM pacientes WHERE id_paciente=?", (id_paciente,)); conn.commit()
        for cid in [c for c, x in self._citas.items() if x.id_paciente == id_paciente]: del self._citas[cid]
        del self._pacientes[id_paciente]; return True
    def agregar_doctor(self, nombre, especialidad, telefono, correo):
        with sqlite3.connect(self.DB_PATH) as conn:
            cursor = conn.cursor()
            cursor.execute("INSERT INTO doctores (nombre,especialidad,telefono,correo) VALUES (?,?,?,?)",
                           (nombre, especialidad, telefono, correo))
            conn.commit(); nuevo_id = cursor.lastrowid
        d = Doctor(nuevo_id, nombre, especialidad, telefono, correo)
        self._doctores[nuevo_id] = d; return d
    def listar_doctores(self): return list(self._doctores.values())
    def obtener_doctor(self, id_doctor): return self._doctores.get(id_doctor)
    def actualizar_doctor(self, id_doctor, nombre, especialidad, telefono, correo):
        if id_doctor not in self._doctores: return False
        with sqlite3.connect(self.DB_PATH) as conn:
            conn.execute("UPDATE doctores SET nombre=?,especialidad=?,telefono=?,correo=? WHERE id_doctor=?",
                         (nombre, especialidad, telefono, correo, id_doctor)); conn.commit()
        self._doctores[id_doctor] = Doctor(id_doctor, nombre, especialidad, telefono, correo); return True
    def eliminar_doctor(self, id_doctor):
        if id_doctor not in self._doctores: return False
        with sqlite3.connect(self.DB_PATH) as conn:
            conn.execute("DELETE FROM citas WHERE id_doctor=?", (id_doctor,))
            conn.execute("DELETE FROM doctores WHERE id_doctor=?", (id_doctor,)); conn.commit()
        for cid in [c for c, x in self._citas.items() if x.id_doctor == id_doctor]: del self._citas[cid]
        del self._doctores[id_doctor]; return True
    def agendar_cita(self, id_paciente, id_doctor, fecha, hora, motivo):
        with sqlite3.connect(self.DB_PATH) as conn:
            cursor = conn.cursor()
            cursor.execute("INSERT INTO citas (id_paciente,id_doctor,fecha,hora,motivo,estado) VALUES (?,?,?,?,?,'Pendiente')",
                           (id_paciente, id_doctor, fecha, hora, motivo))
            conn.commit(); nuevo_id = cursor.lastrowid
        c = Cita(nuevo_id, id_paciente, id_doctor, fecha, hora, motivo)
        self._citas[nuevo_id] = c; return c
    def listar_citas_detalle(self):
        ...
        resultado.sort(key=lambda x: (x["fecha"], x["hora"])); return resultado
    def actualizar_estado_cita(self, id_cita, nuevo_estado):
        if id_cita not in self._citas or nuevo_estado not in Cita.ESTADOS: return False
        with sqlite3.connect(self.DB_PATH) as conn:
            conn.execute("UPDATE citas SET estado=? WHERE id_cita=?", (nuevo_estado, id_cita)); conn.commit()
        self._citas[id_cita].estado = nuevo_estado; return True
    def eliminar_cita(self, id_cita):
        if id_cita not in self._citas: return False
        with sqlite3.connect(self.DB_PATH) as conn:
            conn.execute("DELETE FROM citas WHERE id_cita=?", (id_cita,)); conn.commit()
        del self._citas[id_cita]; return True
    def estadisticas(self):
        especialidades_activas = {self._doctores[c.id_doctor].especialidad
            for c in self._citas.values() if c.id_doctor in self._doctores}
        conteo_estados = {estado: 0 for estado in Cita.ESTADOS}
        for c in self._citas.values(): conteo_estados[c.estado] = conteo_estados.get(c.estado, 0) + 1
        return {"total_pacientes": len(self._pacientes), "total_doctores": len(self._doctores),
                "total_citas": len(self._citas), "especialidades_activas": list(especialidades_activas),
...
def registro():
    ...
    if request.method == 'POST':
        ...
        if len(password) < 6:
            ...
            return render_template('auth/registro.html', roles=ROLES)
        exito, mensaje = registrar_usuario_mysql(nombre, mail, password, rol)
        if exito:
            flash(f'✅ {mensaje} Ya puedes iniciar sesión.', 'success')
...
def login():
    ...
    if request.method == 'POST':
        ...
        datos = autenticar_usuario_mysql(mail, password)
        if datos:
            usuario = Usuario(id_usuario=datos['id_usuario'], nombre=datos['nombre'],
                              mail=datos['mail'], password=datos['password'],
                              rol=datos.get('rol', 'recepcionista'))
...
@login_required
def index():
    return render_template('index.html', stats=gestor.estadisticas(),
                           citas_recientes=gestor.listar_citas_detalle()[:5])
...
def pacientes():
    ...
    return render_template('pacientes/lista.html', pacientes=[p.to_dict() for p in lista], busqueda=busqueda)
...
def paciente_nuevo():
    if request.method == 'POST':
        try:
            gestor.agregar_paciente(nombre=request.form['nombre'], cedula=request.form['cedula'],
...
def paciente_editar(id):
    ...
    if not paciente: flash('Paciente no encontrado.', 'warning'); return redirect(url_for('pacientes'))
    if request.method == 'POST':
        try:
            gestor.actualizar_paciente(id_paciente=id, nombre=request.form['nombre'],
                                       telefono=request.form['telefono'], correo=request.form['correo'])
            flash('✅ Paciente actualizado.', 'success'); return redirect(url_for('pacientes'))
        except Exception as e: flash(f'❌ Error: {e}', 'danger')
...
def paciente_eliminar(id):
    gestor.eliminar_paciente(id); flash('🗑️ Paciente eliminado.', 'success')
    return redirect(url_for('pacientes'))

@app.route('/doctores')
@login_required
def doctores():
    return render_template('doctores/lista.html', doctores=[d.to_dict() for d in gestor.listar_doctores()])
...
def doctor_nuevo():
    if request.method == 'POST':
        try:
            gestor.agregar_doctor(nombre=request.form['nombre'], especialidad=request.form['especialidad'],
                                  telefono=request.form['telefono'], correo=request.form['correo'])
            flash('✅ Doctor registrado correctamente.', 'success'); return redirect(url_for('doctores'))
        except Exception as e: flash(f'❌ Error: {e}', 'danger')
...
def doctor_editar(id):
    ...
    if not doctor: flash('Doctor no encontrado.', 'warning'); return redirect(url_for('doctores'))
    if request.method == 'POST':
        try:
            gestor.actualizar_doctor(id_doctor=id, nombre=request.form['nombre'],
                                     especialidad=request.form['especialidad'],
                                     telefono=request.form['telefono'], correo=request.form['correo'])
            flash('✅ Doctor actualizado.', 'success'); return redirect(url_for('doctores'))
        except Exception as e: flash(f'❌ Error: {e}', 'danger')
...
def doctor_eliminar(id):
    gestor.eliminar_doctor(id); flash('🗑️ Doctor eliminado.', 'success')
    return redirect(url_for('doctores'))

@app.route('/citas')
@login_required
def citas():
    return render_template('citas/lista.html', citas=gestor.listar_citas_detalle(), estados=Cita.ESTADOS)
...
def cita_nueva():
    if request.method == 'POST':
        try:
            gestor.agendar_cita(id_paciente=int(request.form['id_paciente']),
                                id_doctor=int(request.form['id_doctor']),
                                fecha=request.form['fecha'], hora=request.form['hora'],
                                motivo=request.form['motivo'])
            flash('✅ Cita agendada correctamente.', 'success'); return redirect(url_for('citas'))
        except Exception as e: flash(f'❌ Error: {e}', 'danger')
...
@login_required
def cita_estado(id):
    gestor.actualizar_estado_cita(id, request.form.get('estado'))
    flash('✅ Estado actualizado.', 'success'); return redirect(url_for('citas'))

@app.route('/citas/eliminar/<int:id>', methods=['POST'])
@login_required
def cita_eliminar(id):
    gestor.eliminar_cita(id); flash('🗑️ Cita eliminada.', 'success')
...
def datos_ver():
    ...
    return render_template('datos.html', datos_txt=archivos['txt'], datos_json=archivos['json'],
                           datos_csv=archivos['csv'], datos_bd=obtener_datos_bd(),
                           especialidades=Doctor.ESPECIALIDADES_VALIDAS)
...
def datos_guardar():
    try:
        registrar_cita_completa(nombre=request.form['nombre'], cedula=request.form['cedula'],
                                doctor=request.form['doctor'], especialidad=request.form['especialidad'],
                                fecha=request.form['fecha'], hora=request.form['hora'],
...
@login_required
def mysql_usuarios():
    return render_template('mysql/usuarios/lista.html', usuarios=obtener_usuarios())
...
def mysql_usuario_nuevo():
    ...
    if request.method == 'POST':
        try:
            insertar_usuario(nombre=request.form['nombre'], mail=request.form['mail'],
                             password=request.form['password'], rol=request.form.get('rol', 'recepcionista'))
            flash('✅ Usuario creado en MySQL.', 'success'); return redirect(url_for('mysql_usuarios'))
        except Exception as e: flash(f'❌ Error: {e}', 'danger')
    return render_template('mysql/usuarios/formulario.html', usuario=None, accion='Registrar', roles=ROLES)

@app.route('/mysql/usuarios/editar/<int:id>', methods=['GET', 'POST'])
@login_required
def mysql_usuario_editar(id):
    ROLES = ['admin', 'recepcionista', 'medico']
    usuario = next((u for u in obtener_usuarios() if u['id_usuario'] == id), None)
    if not usuario: flash('Usuario no encontrado.', 'warning'); return redirect(url_for('mysql_usuarios'))
    if request.method == 'POST':
        try:
            actualizar_usuario(id_usuario=id, nombre=request.form['nombre'],
                               mail=request.form['mail'], rol=request.form.get('rol', 'recepcionista'))
            flash('✅ Usuario actualizado.', 'success'); return redirect(url_for('mysql_usuarios'))
        except Exception as e: flash(f'❌ Error: {e}', 'danger')
    return render_template('mysql/usuarios/formulario.html', usuario=usuario, accion='Actualizar', roles=ROLES)

@app.route('/mysql/usuarios/eliminar/<int:id>', methods=['POST'])
@login_required
def mysql_usuario_eliminar(id):
    eliminar_usuario(id); flash('🗑️ Usuario eliminado.', 'success')
    return redirect(url_for('mysql_usuarios'))

@app.route('/mysql/pacientes')
@login_required
def mysql_pacientes():
    return render_template('mysql/pacientes/lista.html', pacientes=obtener_pacientes_mysql())
...
def mysql_paciente_nuevo():
    if request.method == 'POST':
        try:
            insertar_paciente_mysql(nombre=request.form['nombre'], cedula=request.form['cedula'],
                                    telefono=request.form['telefono'], correo=request.form['correo'],
                                    fecha_nacimiento=request.form['fecha_nacimiento'])
            flash('✅ Paciente guardado en MySQL.', 'success'); return redirect(url_for('mysql_pacientes'))
        except Exception as e: flash(f'❌ Error: {e}', 'danger')
    return render_template('mysql/pacientes/formulario.html', paciente=None, accion='Registrar')

@app.route('/mysql/pacientes/editar/<int:id>', methods=['GET', 'POST'])
@login_required
def mysql_paciente_editar(id):
    paciente = obtener_paciente_mysql(id)
    if not paciente: flash('Paciente no encontrado.', 'warning'); return redirect(url_for('mysql_pacientes'))
    if request.method == 'POST':
        try:
            actualizar_paciente_mysql(id_paciente=id, nombre=request.form['nombre'],
                                      telefono=request.form['telefono'], correo=request.form['correo'])
            flash('✅ Paciente actualizado en MySQL.', 'success'); return redirect(url_for('mysql_pacientes'))
        except Exception as e: flash(f'❌ Error: {e}', 'danger')
    return render_template('mysql/pacientes/formulario.html', paciente=paciente, accion='Actualizar')

@app.route('/mysql/pacientes/eliminar/<int:id>', methods=['POST'])
@login_required
def mysql_paciente_eliminar(id):
    eliminar_paciente_mysql(id); flash('🗑️ Paciente eliminado de MySQL.', 'success')
    return redirect(url_for('mysql_pacientes'))

@app.route('/mysql/doctores')
@login_required
def mysql_doctores():
    return render_template('mysql/doctores/lista.html', doctores=obtener_doctores_mysql())
...
def mysql_doctor_nuevo():
    if request.method == 'POST':
        try:
            insertar_doctor_mysql(nombre=request.form['nombre'], especialidad=request.form['especialidad'],
                                  telefono=request.form['telefono'], correo=request.form['correo'])
            flash('✅ Doctor guardado en MySQL.', 'success'); return redirect(url_for('mysql_doctores'))
        except Exception as e: flash(f'❌ Error: {e}', 'danger')
    return render_template('mysql/doctores/formulario.html', doctor=None, accion='Registrar',
                           especialidades=Doctor.ESPECIALIDADES_VALIDAS)
...
def mysql_doctor_editar(id):
    ...
    if not doctor: flash('Doctor no encontrado.', 'warning'); return redirect(url_for('mysql_doctores'))
    if request.method == 'POST':
        try:
            actualizar_doctor_mysql(id_doctor=id, nombre=request.form['nombre'],
                                    especialidad=request.form['especialidad'],
                                    telefono=request.form['telefono'], correo=request.form['correo'])
            flash('✅ Doctor actualizado en MySQL.', 'success'); return redirect(url_for('mysql_doctores'))
        except Exception as e: flash(f'❌ Error: {e}', 'danger')
    return render_template('mysql/doctores/formulario.html', doctor=doctor, accion='Actualizar',
                           especialidades=Doctor.ESPECIALIDADES_VALIDAS)

@app.route('/mysql/doctores/eliminar/<int:id>', methods=['POST'])
@login_required
def mysql_doctor_eliminar(id):
    eliminar_doctor_mysql(id); flash('🗑️ Doctor eliminado de MySQL.', 'success')
    return redirect(url_for('mysql_doctores'))

@app.route('/mysql/citas')
@login_required
def mysql_citas():
    return render_template('mysql/citas/lista.html', citas=obtener_citas_detalle_mysql(), estados=Cita.ESTADOS)
...
def mysql_cita_nueva():
    if request.method == 'POST':
        try:
            insertar_cita_mysql(id_paciente=int(request.form['id_paciente']),
                                id_doctor=int(request.form['id_doctor']),
                                fecha=request.form['fecha'], hora=request.form['hora'],
                                motivo=request.form['motivo'])
            flash('✅ Cita agendada en MySQL.', 'success'); return redirect(url_for('mysql_citas'))
        except Exception as e: flash(f'❌ Error: {e}', 'danger')
    return render_template('mysql/citas/formulario.html',
                           pacientes=obtener_pacientes_mysql(), doctores=obtener_doctores_mysql())

@app.route('/mysql/citas/estado/<int:id>', methods=['POST'])
@login_required
def mysql_cita_estado(id):
    actualizar_estado_cita_mysql(id, request.form.get('estado'))
    flash('✅ Estado actualizado.', 'success'); return redirect(url_for('mysql_citas'))

@app.route('/mysql/citas/eliminar/<int:id>', methods=['POST'])
@login_required
def mysql_cita_eliminar(id):
    eliminar_cita_mysql(id); flash('🗑️ Cita eliminada de MySQL.', 'success')
